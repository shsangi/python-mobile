...
def show_single_frame_preview(video_path, time_point=1):
    ...
    try:
        ...
        preview_width = 200
        preview_height = 355  # 9:16 aspect ratio
...
def fit_content_to_portrait(clip):
    ...
    try:
        ...
        bg_clip = clip.resize((target_width, target_height)).fl_image(
            lambda img: np.array(Image.fromarray(img).filter(ImageFilter.GaussianBlur(radius=20)))
        )
